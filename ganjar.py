...
label2 = tk.Label(frame1, text="Halo, ini Muhammad Sopyan XII PPLG 1", textvariable=input_text, bg="gray", anchor="e", justify="right", font=("Arial", 32), fg="white")  # Mengganti warna teks menjadi putih
label2.grid(row=1, column=0, sticky="nsew", padx=20, pady=20)

button_font = ("Arial", 18)

buttons = [
    ("C", bt_clear, 0, 0), ("DEL", bt_del, 0, 1), ("/", lambda: tombol_klik("/"), 0, 2), 
    ("×", lambda: tombol_klik("*"), 0, 3),
    ...
    ("1", lambda: tombol_klik("1"), 3, 0), ("2", lambda: tombol_klik("2"), 3, 1), ("3", lambda: tombol_klik("3"), 3, 2), ("=", bt_equal, 3, 3),
    ("0", lambda: tombol_klik("0"), 4, 0, 2), (".", lambda: tombol_klik("."), 4, 2), ("%", lambda: tombol_klik("/100"), 4, 3)
]

for (text, command, row, col, *colspan) in buttons:
    button = tk.Button(frame2, text=text, command=command, 
                       bg="grey" if text in ["+", "-", "*", "/", "C", "DEL", "="] else ("gray" if text == "×" else None), 
