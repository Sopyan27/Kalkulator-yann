...
def bt_negatif_positif():
    global expression
    if expression and expression[0] == "-":
        expression = expression[1:]  # Hilangkan tanda negatif
    else:
        expression = "-" + expression  # Tambahkan tanda negatif
    input_text.set(expression)
...
buttons = [
    ("C", bt_clear, 0, 0), ("+/-", bt_negatif_positif, 0, 1), ("/", lambda: tombol_klik("/"), 0, 3), 
...
for (text, command, row, col, *colspan) in buttons:
    button = tk.Button(frame2, text=text, command=command, 
                       bg="grey" if text in ["+", "-", "*", "/", "C", "+/-", "×", "=","%"] else None, 
                       fg="white" if text in ["+", "-", "*", "/", "C", "+/-", "×", "=","%"] else None, font=button_font)
